...
async def kipris_search_patents(
    ...
    Args:
        applicant_name: 출원인명 (필수, 예: '삼성전자', '카카오뱅크')
            검색 연산자 가이드:
              - 공백 또는 *: AND 검색 (모든 단어 포함) - 예: "MPC 지갑" 또는 "MPC*지갑"
              - +: OR 검색 (하나라도 포함) - 예: "MPC+지갑+분산"
              - !: NOT 검색 (제외, AND와 함께 사용) - 예: "자동차*!엔진"
              - "": 구문검색 (순서대로 인접하여 나열) - 예: '"데이터 신호"'
              - ^1~3: NEAR 검색 (단어 거리 1~3) - 예: "자동차^2각도"
        page: 페이지 번호 (기본값: 1)
