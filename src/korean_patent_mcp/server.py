...
async def kipris_search_patents(
    ...
    Args:
        applicant_name: 출원인명 (필수, 예: '삼성전자', '카카오뱅크')
            검색 연산자 가이드:
              - *: AND 검색 (입력된 키워드가 모두 포함) - 예: "휴대폰*케이스"
              - +: OR 검색 (입력된 키워드 중 하나라도 포함) - 예: "휴대폰+케이스"
              - !: NOT 검색 (해당 키워드 제외, 단독 사용 불가, AND와 함께 사용) - 예: "자동차*!엔진"
              - "": 구문검색 (검색어가 순서대로 인접하여 나열, 공백/복합명사/조사/특수문자 포함 가능) - 예: \"데이터 신호\"
              - ^1~3: NEAR 검색 (단어 거리 1~3, 단어 순서 고려) - 예: "자동차^2각도"
        page: 페이지 번호 (기본값: 1)
